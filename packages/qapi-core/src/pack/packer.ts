...
/**
 * swagger
 * @param {string} swaggerStr
 */
export const packer = async (swaggerStr: string) => {
  ...
  doc = doc
    .replace(/\{\{name\}\}/m, swaggerObj.info.title || '')
    .replace(/\{\{version\}\}/m, swaggerObj.info.version || '')
    .replace(/\{\{description\}\}/m, (swaggerObj.tags || []).map(t => t.name).join(','))
...
export const pack = (opt: Opts): void => {
  try {
    cp.execSync(`npm publish --registry=${opt.registry} --scope=@${opt.scope ? '': null}`)
  } catch (e) {
    debug('ERR', e)
  }
}
